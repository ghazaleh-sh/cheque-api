...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
            ...
            .map(f ->
                new FieldErrorVM(
                    ...
                    StringUtils.isNotBlank(f.getDefaultMessage())
                        ? messageSource.getMessage(f.getDefaultMessage(), null, request.getLocale())
                        : f.getCode()
                )
            )
    ...
    public ResponseEntity<Problem> generalException(GeneralException ex, NativeWebRequest request) {
        String message = "";
        try {
            message = messageSource.getMessage(ex.getMessage(), null, request.getLocale());
        } catch (Exception e) {
            message = messageSource.getMessage("server.external.exception.not.translate", null, request.getLocale());
        }
        Problem problem = Problem
            .builder()
            .withType(ErrorConstants.CONSTRAINT_VIOLATION_TYPE)
